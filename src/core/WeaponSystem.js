...
function findBone(model, baseName) {
    ...
    const variations = [
        baseName,                  // Plain name (mixamorigRightHand)
        'Armature_' + baseName,    // With Armature_ prefix
        baseName.replace('mixamorig', ''), // Without mixamorig (RightHand)
...
class WeaponSystem {
	...
	*/
	init() {
		try {
			...
			console.error('Error initializing WeaponSystem:', error);
	...
	changeWeapon( type ) {
		...
		if ( weapon ) {
			...
			switch ( weapon.type ) {
				...
				default:
					console.error( 'RIFT.WeaponSystem: Invalid weapon type:', type );
					break;
	...
	*/
	setNextWeapon( type ) {
		...
		if ( this.currentWeapon && this.currentWeapon.type !== type ) {
	...
	*/
	showCurrentWeapon() {
		if (!this.currentWeapon) return this;
	...
	*/
	_initRenderComponents() {
		try {
			...
			console.error('Error initializing render components:', error);
			return this;
		}
	}
	...
	_initBlasterRenderComponent() {
		try {
			...
			if (this.owner.isPlayer === false) {
				...
				const rightHand = findBone(this.owner._renderComponent, 'mixamorigRightHand');
				if (rightHand) {
					rightHand.add(blasterMesh);
				} else {
					console.warn('Right hand bone not found for attaching blaster');
				}
			...
			console.error('Error initializing blaster render component:', error);
			return this;
		}
	}
		...
		try {
			...
			if (this.owner.isPlayer === false) {
				...
				if (rightHand) {
					...
					console.warn('Right hand bone not found for attaching shotgun');
				}
			...
			console.error('Error initializing shotgun render component:', error);
			return this;
		}
	}
		...
		try {
			...
			if (this.owner.isPlayer === false) {
				...
				if (rightHand) {
					...
					console.warn('Right hand bone not found for attaching assault rifle');
				}
			...
			console.error('Error initializing assault rifle render component:', error);
			return this;
		}
	}
