	...
	*/
	update( delta ) {
		...
		if ( this.status === STATUS_DEAD ) {
			...
			if ( this.world.debug ) console.log( 'RIFT.Player: Player died.' );
	...
	*/
	addHealth( amount ) {
		...
		if ( this.world.debug ) {
			...
			console.log( 'RIFT.Player: Entity with ID %s receives %i health points.', this.uuid, amount );
		...
		switch ( telegram.message ) {
			...
			case MESSAGE_HIT:
				...
				if ( this.world.debug ) {
					...
					console.log( 'RIFT.Player: Player hit by Game Entity with ID %s receiving %i damage.', telegram.sender.uuid, telegram.data.damage );
