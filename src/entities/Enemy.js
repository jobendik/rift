	...
	update( delta ) {
		...
		if ( this.status === STATUS_DEAD ) {
			...
			if ( this.world.debug ) {
				...
				console.log( 'RIFT.Enemy: Enemy with ID %s died.', this.uuid );
	...
	*/
	addHealth( amount ) {
		...
		if ( this.world.debug ) {
			...
			console.log( 'RIFT.Enemy: Entity with ID %s receives %i health points.', this.uuid, amount );
	...
	*/
	ignoreItem( type ) {
		...
		switch ( type ) {
			...
			default:
				console.error( 'RIFT.Enemy: Invalid item type:', type );
				break;
		...
		switch ( telegram.message ) {
			...
			case MESSAGE_HIT:
				...
				if ( this.world.debug ) {
					...
					console.log( 'RIFT.Enemy: Enemy with ID %s hit by Game Entity with ID %s receiving %i damage.', this.uuid, telegram.sender.uuid, telegram.data.damage );
