 ...
 *
 * An optimized version of DamageNumbers that uses the ElementPool utility
...
class EnhancedDamageNumbers extends UIComponent {
    /**
     * Create a new EnhancedDamageNumbers component
     * 
    ...
    constructor(options = {}) {
        ...
        this.maxNumbers = options.maxNumbers || UIConfig.damageNumbers.maxNumbers || 30;
        this.duration = options.duration || UIConfig.damageNumbers.duration * 1000 || 1500;
        this.stackThreshold = options.stackThreshold || UIConfig.damageNumbers.stackThreshold || 300;
        this.riseDistance = options.riseDistance || UIConfig.damageNumbers.riseDistance || 30;
        this.stackLimit = options.stackLimit || UIConfig.damageNumbers.stackLimit || 5;
    ...
    init() {
        ...
        // Initialize the element pool for damage numbers
     ...
     */
    _initElementPool() {
        // Create custom element creation function
        const createFn = () => {
        ...
        // Create custom reset function
        const resetFn = (element) => {
        ...
        this.pool = new ElementPool({
            ...
            maxSize: this.maxNumbers * 1.5, // Allow some buffer
            useBlocks: true,
            blockSize: 10
        });
    }
    ...
    showDamageNumber(options = {}) {
        ...
        if (!element) return this; // No available numbers
        ...
        element.textContent = damageText;
        element.className = `rift-damage-number ${damageClass}`;
        element.style.setProperty('--rift-damage-number-rise', `${this.riseDistance}px`);
        element.style.setProperty('--rift-damage-number-duration', `${duration || this.duration}ms`);
        ...
        // Activate the number
        element.classList.add('rift-damage-number--active');
    ...
    _showStackedNumber(stackInfo) {
        ...
        if (stackInfo.hits.length > 1) {
            ...
            element.textContent = String(Math.round(stackInfo.totalDamage));
        }
        
        // Set CSS classes
        element.className = `rift-damage-number ${highestPriorityClass}`;
        element.style.setProperty('--rift-damage-number-rise', `${this.riseDistance}px`);
        element.style.setProperty('--rift-damage-number-duration', `${this.duration}ms`);
        
        // Create damage number object
        const number = {
            startTime: performance.now(),
            duration: this.duration,
            damage: stackInfo.totalDamage,
            targetId: stackInfo.targetId,
            element,
            release
        };
    ...
    _registerEventListeners() {
        // Register for hit events using the standardized event names
        this.registerEvents({
            'hit:registered': this._onHitLanded,
