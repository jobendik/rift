...
foreach ($file in $jsFiles) {
    ...
    $defaultExportPattern = 'export default (\w+);?'
    if ($content -match $defaultExportPattern) {
        $className = $Matches[1]
        ...
        if ($shouldBeNamedExports -contains $className -or $className -eq $filename) {
            ...
            $changes += 'Default to Named export: ' + $className
            $stats.ExportsModernized++
        }
    }
    
    # Fix 2: Fix export class declarations
    $exportClassPattern = 'export class (\w+)'
    if ($content -match $exportClassPattern) {
        $className = $Matches[1]
        $exportPattern = 'export \{ ' + $className + ' \}'
        if ($content -notmatch [regex]::Escape($exportPattern)) {
            $content = $content -replace "export class $className", "class $className"
            $content = $content + "`r`n`r`nexport { $className };"
            $changes += 'Class export to Named: ' + $className
            $stats.ExportsModernized++
        }
    }
...
foreach ($file in $jsFiles) {
    ...
    foreach ($wrong in $modernImportFixes.Keys) {
        $right = $modernImportFixes[$wrong]
        $wrongEscaped = [regex]::Escape($wrong)
        if ($content -match $wrongEscaped) {
            $content = $content -replace $wrongEscaped, $right
            $moduleName = $wrong.Split(' ')[1]
            $changes += 'Import modernized: ' + $moduleName
            $stats.ImportsFixed++
        }
    }
    
    # Apply utility import fixes  
    foreach ($wrong in $keepNamedImports.Keys) {
        $right = $keepNamedImports[$wrong]
        $wrongEscaped = [regex]::Escape($wrong)
        if ($content -match $wrongEscaped) {
            $content = $content -replace $wrongEscaped, $right
            $moduleName = $wrong.Split(' ')[1]
            $changes += 'Utility import fixed: ' + $moduleName
            $stats.ImportsFixed++
        }
    }
    
    # Apply base class import fixes
    foreach ($wrong in $keepDefaultImports.Keys) {
        $right = $keepDefaultImports[$wrong]
        $wrongEscaped = [regex]::Escape($wrong)
        if ($content -match $wrongEscaped) {
            $content = $content -replace $wrongEscaped, $right
            $moduleName = $wrong -replace 'import \{ ' -replace ' \} from'
            $changes += 'Base class import fixed: ' + $moduleName
            $stats.ImportsFixed++
        }
    }
